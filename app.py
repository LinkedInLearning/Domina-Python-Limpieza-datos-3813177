...
datos_codificados = codificador.fit_transform(df[['tipo_prop']]).toarray()
print(datos_codificados)

df_datos_codificados = pd.DataFrame(datos_codificados, columns=codificador.get_feature_names_out(['tipo_prop']))

df = pd.concat([df, df_datos_codificados], axis=1)
