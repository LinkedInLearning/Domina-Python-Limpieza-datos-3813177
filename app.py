...
caracteristicas_escaladas = escalar.fit_transform(df[columnas_numericas])

df_escalado = pd.DataFrame(caracteristicas_escaladas, columns=columnas_numericas)

df = pd.concat([df.drop(columns=columnas_numericas), df_escalado], axis=1)
